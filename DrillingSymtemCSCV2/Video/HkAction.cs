...
namespace DrillingSymtemCSCV2
{
    ...
    {        
        ...
        private static string AppKey = ConfigurationManager.AppSettings["AppKey"].ToString();//"599806f22c9b4f8cbc552070cc8a08d0";
        private static string AuthAddr = "https://auth.ys7.com";
        public static bool IsLoaded = false;//记录登录状态
        private static string PhoneNumber = ConfigurationManager.AppSettings["PhoneNumber"].ToString();//"15633378999";
        private static string PlatformAddr = "https://open.ys7.com";
        private static string SecretKey = ConfigurationManager.AppSettings["SecretKey"].ToString();//"a80969fc9f5b1924e4f6998e9f3632c8";
        ...
        public static string GetAccessToken()
        {
            ...
            int ret = HkSDK.OpenSDK_HttpSendWithWait(ApiUrl, jsonStr, "", out iMessage, out iLength);
        ...
        public static int HandlerWork(IntPtr SessionId, uint MsgType, uint Error, string Info, IntPtr pUser)
        {
            switch (MsgType)
            {
                case 20:
                    ...
                    if (Error == 0)
                    {
                 //       Frm_Main.jObjInfo = obj;
                  //      Frm_Main.isOpertion = 2;
                    }
                    break;
                case 3:// 播放开始
                    break;
                ...
                case 5:// 播放结束，回放结束时会有此消息
                   // Frm_Main.mType = Frm_Main.MessageType.INS_PLAY_ARCHIVE_END;
                    break;
                default:
                    break;
            }
